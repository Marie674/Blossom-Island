...
namespace PixelCrushers.QuestMachine
{
    ...
    public static class QuestEditorUtility
    {
        ...
        public static void ArrangeNodes(Quest quest)
        {
            ...
            tree.Add(new List<int>(new int[] { 0 })); // Start node.
            var unassigned = new List<int>();
            for (int i = 1; i < quest.nodeList.Count; i++)
            {
                unassigned.Add(i);
            }
            int maxLevelSize = ArrangeNodesBuildTree(quest, tree, unassigned, 0);
            ...
            for (int i = 0; i < tree.Count; i++)
            {
                ...
                for (int j = 0; j < level.Count; j++)
                {
                    ...
                    node.canvasRect = new Rect(nodeLeft, top, node.canvasRect.width, node.canvasRect.height);
                }
            }
        }

        private static int ArrangeNodesBuildTree(Quest quest, List<List<int>> tree, List<int> unassigned, int safeguard)
        {
            ...
            {
                tree.Add(children);
                return Mathf.Max(level.Count, ArrangeNodesBuildTree(quest, tree, unassigned, safeguard++));
            }
            else
            {
                return level.Count;
