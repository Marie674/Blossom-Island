...
public partial class ItemDatabaseEditorWindow : EditorWindow
{
	...
	SerializedProperty autoGeneric;
	SerializedProperty autoTool;
	SerializedProperty autoBottle;
	SerializedProperty autoFood;
	SerializedProperty autoPlaceableItem;
	SerializedProperty autoGridItem;
	SerializedProperty autoMaterial;
	SerializedProperty autoBuilding;
	SerializedProperty autoRecipe;
    //#VID-SP
    ...
    void OnEnable()
    {
		...
		autoGeneric = vidListsSerialized.FindProperty("autoGeneric");
		autoTool = vidListsSerialized.FindProperty("autoTool");
		autoBottle = vidListsSerialized.FindProperty("autoBottle");
		autoFood = vidListsSerialized.FindProperty("autoFood");
		autoPlaceableItem = vidListsSerialized.FindProperty("autoPlaceableItem");
		autoGridItem = vidListsSerialized.FindProperty("autoGridItem");
		autoMaterial = vidListsSerialized.FindProperty("autoMaterial");
		autoBuilding = vidListsSerialized.FindProperty("autoBuilding");
		autoRecipe = vidListsSerialized.FindProperty("autoRecipe");
        //Get a serialized object and a property for the items list
        //#VID-ASPE

        //#VID-SPLAB
		itemLists[(int)ItemType.Generic] = autoGeneric;
		itemLists[(int)ItemType.Tool] = autoTool;
		itemLists[(int)ItemType.Bottle] = autoBottle;
		itemLists[(int)ItemType.Food] = autoFood;
		itemLists[(int)ItemType.PlaceableItem] = autoPlaceableItem;
		itemLists[(int)ItemType.GridItem] = autoGridItem;
		itemLists[(int)ItemType.Material] = autoMaterial;
		itemLists[(int)ItemType.Building] = autoBuilding;
		itemLists[(int)ItemType.Recipe] = autoRecipe;
        //#VID-SPLAE
            ...
            if (!confirmBeforeDeleting || ItemRemovalConfirmed())
            {
                ...
                switch (itemToShow.listIndex)
                {
					case (int)ItemType.Generic:
						autoVidLists.autoGeneric.RemoveAt(index);
						break;
					case (int)ItemType.Tool:
						autoVidLists.autoTool.RemoveAt(index);
						break;
					case (int)ItemType.Bottle:
					autoVidLists.autoBottle.RemoveAt(index);
						break;
					case (int)ItemType.Food:
						autoVidLists.autoFood.RemoveAt(index);
						break;
					case (int)ItemType.PlaceableItem:
						autoVidLists.autoPlaceableItem.RemoveAt(index);
						break;
					case (int)ItemType.GridItem:
					autoVidLists.autoGridItem.RemoveAt(index);
						break;
					case (int)ItemType.Material:
						autoVidLists.autoMaterial.RemoveAt(index);
						break;
					case (int)ItemType.Building:
						autoVidLists.autoBuilding.RemoveAt(index);
						break;
					case (int)ItemType.Recipe:
						autoVidLists.autoRecipe.RemoveAt(index);
						break;
                }//#VID-MSB
        ...
        else if (GUILayout.Button(new GUIContent("+", "Duplicate Item"), EditorStyles.miniButtonRight, GUILayout.Width(20)))
        {
            ...
            switch (itemToDuplicate.itemType)
            {
				case ItemType.Generic:
					autoVidLists.autoGeneric.Insert(index, (ItemBase)itemCont.item);
					break;
				case ItemType.Tool:
					autoVidLists.autoTool.Insert(index, (ItemTool)itemCont.item);
					break;
				case ItemType.Bottle:
					autoVidLists.autoBottle.Insert(index, (ItemBottle)itemCont.item);
					break;
				case ItemType.Food:
					autoVidLists.autoFood.Insert(index, (ItemFood)itemCont.item);
					break;
				case ItemType.PlaceableItem:
					autoVidLists.autoPlaceableItem.Insert(index, (ItemPlaceable)itemCont.item);
					break;
			case ItemType.GridItem:
				autoVidLists.autoGridItem.Insert(index, (ItemGrid)itemCont.item);
					break;
				case ItemType.Material:
					autoVidLists.autoMaterial.Insert(index, (ItemMaterial)itemCont.item);
					break;
				case ItemType.Building:
					autoVidLists.autoBuilding.Insert(index, (ItemBuilding)itemCont.item);
					break;
				case ItemType.Recipe:
					autoVidLists.autoRecipe.Insert(index, (ItemRecipe)itemCont.item);
					break;
            }//#VID-2MSB
    ...
    void AddItem()
    {
        ...
        switch (((ItemType)itemToShow.listIndex))
        {
			...
			case ItemType.Generic:
				ItemBase autoGenericVAR = new ItemBase();
				autoGenericVAR.itemID = database.GetNewID(ItemType.Generic);
				autoGenericVAR.itemType = ItemType.Generic;
				
				autoVidLists.autoGeneric.Add(autoGenericVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoGeneric.Count - 1;
				break;

			case ItemType.Tool:
				ItemTool autoToolVAR = new ItemTool();
				autoToolVAR.itemID = database.GetNewID(ItemType.Tool);
				autoToolVAR.itemType = ItemType.Tool;
				
				autoVidLists.autoTool.Add(autoToolVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoTool.Count - 1;
				break;

			case ItemType.Bottle:
				ItemBottle autoBottleVAR = new ItemBottle();
				autoBottleVAR.itemID = database.GetNewID(ItemType.Bottle);
				autoBottleVAR.itemType = ItemType.Bottle;
				
				autoVidLists.autoBottle.Add(autoBottleVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoBottle.Count - 1;
				break;

			case ItemType.Food:
				ItemFood autoFoodVAR = new ItemFood();
				autoFoodVAR.itemID = database.GetNewID(ItemType.Food);
				autoFoodVAR.itemType = ItemType.Food;
				
				autoVidLists.autoFood.Add(autoFoodVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoFood.Count - 1;
				break;

			case ItemType.PlaceableItem:
				ItemPlaceable autoPlaceableItemVAR = new ItemPlaceable();
				autoPlaceableItemVAR.itemID = database.GetNewID(ItemType.PlaceableItem);
				autoPlaceableItemVAR.itemType = ItemType.PlaceableItem;
				
				autoVidLists.autoPlaceableItem.Add(autoPlaceableItemVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoPlaceableItem.Count - 1;
				break;

		case ItemType.GridItem:
			ItemGrid autoSeedVAR = new ItemGrid();
			autoSeedVAR.itemID = database.GetNewID(ItemType.GridItem);
			autoSeedVAR.itemType = ItemType.GridItem;
				
			autoVidLists.autoGridItem.Add(autoSeedVAR);
			itemToShow.itemToShowIndex = autoVidLists.autoGridItem.Count - 1;
				break;

			case ItemType.Material:
				ItemMaterial autoMaterialVAR = new ItemMaterial();
				autoMaterialVAR.itemID = database.GetNewID(ItemType.Material);
				autoMaterialVAR.itemType = ItemType.Material;
				
				autoVidLists.autoMaterial.Add(autoMaterialVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoMaterial.Count - 1;
				break;

			case ItemType.Building:
				ItemBuilding autoBuildingVAR = new ItemBuilding();
				autoBuildingVAR.itemID = database.GetNewID(ItemType.Building);
				autoBuildingVAR.itemType = ItemType.Building;
				
				autoVidLists.autoBuilding.Add(autoBuildingVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoBuilding.Count - 1;
				break;

			case ItemType.Recipe:
				ItemRecipe autoRecipeVAR = new ItemRecipe();
				autoRecipeVAR.itemID = database.GetNewID(ItemType.Recipe);
				autoRecipeVAR.itemType = ItemType.Recipe;
				
				autoVidLists.autoRecipe.Add(autoRecipeVAR);
				itemToShow.itemToShowIndex = autoVidLists.autoRecipe.Count - 1;
				break;
        }//#VID-AIE
    ...
    ItemBase GetItemAtIndex(int index, bool mainOnly = false)
    {
        ...
        if (selectedSubtype == 0 || mainOnly)
        {
            ...
            {
			case (int)ItemType.Generic:
				return autoVidLists.autoGeneric[index];
			case (int)ItemType.Tool:
				return autoVidLists.autoTool[index];
			case (int)ItemType.Bottle:
				return autoVidLists.autoBottle[index];
			case (int)ItemType.Food:
				return autoVidLists.autoFood[index];
			case (int)ItemType.PlaceableItem:
				return autoVidLists.autoPlaceableItem[index];
			case (int)ItemType.GridItem:
				return autoVidLists.autoGridItem[index];
			case (int)ItemType.Material:
				return autoVidLists.autoMaterial[index];
			case (int)ItemType.Building:
				return autoVidLists.autoBuilding[index];
			case (int)ItemType.Recipe:
				return autoVidLists.autoRecipe[index];
            }//#VID-GIAIE
        }
    ...
    List<string> GetFormattedNamesList(ItemType t, ref List<string> invalidList)
    {
        ...
        {//#VID-GFNLB
			case ItemType.Generic:
				...
				{
					itemName = autoVidLists.autoGeneric[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoGeneric[i].itemID + ",");
				}
				break;
			case ItemType.Tool:
				for (int i = 0; i < autoVidLists.autoTool.Count; i++)
				{
					itemName = autoVidLists.autoTool[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoTool[i].itemID + ",");
				}
				break;
			case ItemType.Bottle:
				for (int i = 0; i < autoVidLists.autoBottle.Count; i++)
				{
					itemName = autoVidLists.autoBottle[i].itemName.Trim();
					...
					itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoBottle[i].itemID + ",");
				}
				break;
			case ItemType.Food:
				for (int i = 0; i < autoVidLists.autoFood.Count; i++)
				{
					itemName = autoVidLists.autoFood[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoFood[i].itemID + ",");
				}
				break;
			case ItemType.PlaceableItem:
				for (int i = 0; i < autoVidLists.autoPlaceableItem.Count; i++)
				{
					itemName = autoVidLists.autoPlaceableItem[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoPlaceableItem[i].itemID + ",");
				}
				break;
		case ItemType.GridItem:
			for (int i = 0; i < autoVidLists.autoGridItem.Count; i++)
				{
				itemName = autoVidLists.autoGridItem[i].itemName.Trim();
				...
				itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoGridItem[i].itemID + ",");
				}
				break;
			case ItemType.Material:
				for (int i = 0; i < autoVidLists.autoMaterial.Count; i++)
				{
					itemName = autoVidLists.autoMaterial[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoMaterial[i].itemID + ",");
				}
				break;
			case ItemType.Building:
				for (int i = 0; i < autoVidLists.autoBuilding.Count; i++)
				{
					itemName = autoVidLists.autoBuilding[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoBuilding[i].itemID + ",");
				}
				break;
			case ItemType.Recipe:
				for (int i = 0; i < autoVidLists.autoRecipe.Count; i++)
				{
					itemName = autoVidLists.autoRecipe[i].itemName.Trim();
					...
					if (!hash.Add(itemName) || !char.IsLetter(itemName[0]))
					{
						...
						itemNames.Add("\t\t" + itemName.Replace(" ", "") + " = " + autoVidLists.autoRecipe[i].itemID + ",");
				}
				break;
        }//#VID-GFNLE
