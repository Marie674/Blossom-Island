...
public class GameInputManager : Singleton<GameInputManager>
{
    ...
    {
        if (!string.IsNullOrEmpty(axis) && Instance) Instance.observedAxes.Add(axis);
    }

    /// <summary>Register a button as one of interest.</summary>
    public static void ObserveButton(string button)
    {
        if (!string.IsNullOrEmpty(button) && Instance) Instance.observedButtons.Add(button);
    }

    /// <summary>Register a keycode as one of interest.</summary>
    public static void ObserveKeyCode(KeyCode keyCode)
    {
        if (keyCode != KeyCode.None && Instance) Instance.observedKeycodes.Add(keyCode);
    }
    ...
    {
        if (Action != null && Instance != null) Instance.GetBlock(Instance.highestPriority + 1).Event += Action;
    }
    ...
    {
        if (Action != null && Instance != null) Instance.GetBlock(priority).Event += Action;
    }
    ...
    {
        if (Action != null && Instance != null) foreach (EventBlock b in Instance.eventBlocks) b.Event -= Action;
    }
    #endregion
